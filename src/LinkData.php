...
switch($_GET["task"]) {
     ...
     case "insertRow": 
          ...
          if ($stmt = $connect->prepare("INSERT INTO AbaLinks(Name,URL,TypeID,Duration) VALUES(?,?,?,?)")) {
               $stmt->bind_param("ssis", $_GET["Name"],$_GET["URL"],$typeID,$duration);
...
if ($result->num_rows > 0) {
     ...
     while($row = $result->fetch_assoc()) {
          switch($_GET["task"]) {
               case "fetchData":
                    $arr=array('ID' => $row["LinkID"],'Name' => cleanStr($row["Name"]),'URL' => cleanStr($row["URL"]),'TypeID' => $row["TypeID"],'Type' => $row["LinkTypeName"]);
                    break;
               case "fetchTypes":
